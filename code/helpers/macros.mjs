...
export async function createDocMacro(data, slot) {
  ...
  if (!data.uuid.includes("Actor.")) {
    return ui.notifications.warn("HOLLOWS_RPG.Macro.Warnings.Create.NotOwnedItem", { localize: true });
  }
  ...
  switch (item.type) {
    case "ability":
    case "project":
      command = `hollows.helpers.macros.rollItemMacro("${data.uuid}");`;
      name = item.name;
      break;
    default:
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command,
      flags: { "hollows-rpg.itemMacro": true },
    });
  }
