 ...
 * @import { HollowsRPGActiveEffect, HollowsRPGActor, HollowsRPGItem } from "../../documents/_module.mjs";
 */
 ...
 */
export default class HollowsDocumentSheet extends api.HandlebarsApplicationMixin(api.DocumentSheet) {
  /** @inheritdoc */
  static DEFAULT_OPTIONS = {
    ...
    actions: {
      toggleMode: HollowsDocumentSheet.#toggleMode,
      viewDoc: HollowsDocumentSheet.#viewDoc,
      createDoc: HollowsDocumentSheet.#createDoc,
      deleteDoc: HollowsDocumentSheet.#deleteDoc,
      createPseudoDocument: HollowsDocumentSheet.#createPseudoDocument,
      deletePseudoDocument: HollowsDocumentSheet.#deletePseudoDocument,
      renderPseudoDocumentSheet: HollowsDocumentSheet.#renderPseudoDocumentSheet,
      toggleDocumentDescription: HollowsDocumentSheet.#toggleDocumentDescription,
      updateFromCompendium: HollowsDocumentSheet.#updateFromCompendium,
    },
   ...
   * @type {typeof HollowsDocumentSheet.MODES[keyof typeof HollowsDocumentSheet.MODES]}
   * @protected
   */
  _mode = HollowsDocumentSheet.MODES.PLAY;
   ...
   */
  get isPlayMode() {
    return this._mode === HollowsDocumentSheet.MODES.PLAY;
  }
   ...
   */
  get isEditMode() {
    return this._mode === HollowsDocumentSheet.MODES.EDIT;
  }
  ...
  _configureRenderOptions(options) {
    ...
    else if (options.renderContext === `create${this.document.documentName}`) this._mode = HollowsDocumentSheet.MODES.EDIT;
  }
  ...
  async _renderFrame(options) {
    ...
    if (this.document.system.source) {
      buttons.push(constructHTMLButton({
        ...
        dataset: { action: "updateSource", tooltip: "HOLLOWS_RPG.SOURCE.Update" },
      }));
    }
  ...
  static async #toggleMode(event, target) {
    ...
    await this.render({ mode: this.isPlayMode ? HollowsDocumentSheet.MODES.EDIT : HollowsDocumentSheet.MODES.PLAY });
  }
  ...
  static async #createPseudoDocument(event, target) {
    ...
    if (!type && (foundry.utils.isSubclass(Cls, hollows.data.pseudoDocuments.TypedPseudoDocument))) {
  ...
  static async #updateFromCompendium(event, target) {
    await hollows.utils.updateFromCompendium(this.document);
  }
   ...
   * @returns {Promise<HollowsRPGActiveEffect|null>} A Promise resolving to the dropped ActiveEffect (if sorting), a newly created ActiveEffect,
   *                                         or null in case of failure or no action being taken.
   * @protected
   */
   ...
   * @param {DragEvent} event       The initiating drop event.
   * @param {HollowsRPGActor} actor  The dropped Actor document.
   * @returns {Promise<HollowsRPGActor|null>} A Promise resolving to the dropped Actor (if sorting), a newly created Actor,
   *                                         or null in case of failure or no action being taken.
   * @protected
   */
   ...
   * @returns {Promise<HollowsItem|null>} A Promise resolving to the dropped Item (if sorting), a newly created Item,
   *                                         or null in case of failure or no action being taken.
   * @protected
   */
